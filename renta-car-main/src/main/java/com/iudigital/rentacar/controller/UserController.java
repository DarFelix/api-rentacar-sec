...
public class UserController {
	...
	@GetMapping("/{userId}")
	public UserDTO getUser(@PathVariable int userId)   throws Exception {
		//User user = userService.getUserById(userId);
		//return userConverter.convertUserToUserDTO(user);
		
		return userConverter.convertUserToUserDTO(userService.getUserById(userId));
	}
	
	@PutMapping("/{userId}")
	@ResponseStatus(HttpStatus.NO_CONTENT)
	public void  editUser(@PathVariable int userId, @RequestBody User user) throws Exception  {
	...
	@PostMapping("/in/nombres2")
	@ResponseBody
	public List<UserDTO> getUserByNombreIn2(@RequestBody List<UserDTO> dtos){
				List<String> nombres = dtos.stream().map(u -> u.getNombre()).collect(Collectors.toList());
	...
	@PostMapping("/in/nombres3")
	@ResponseBody
	public List<UserDTO> getUserByNombreIn3(@RequestBody List<String> nombres){
